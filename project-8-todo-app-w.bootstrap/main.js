...
function generateTemplate(todo) {
  ...
  <form>
  <input type="text" class="editField inputs" value="${todo.name}" readonly disabled="disabled">
...
ul.addEventListener('submit', e => {
  ...
  const newNameVal = e.target.firstChild.nextSibling.value;
  const parentId = e.target.parentElement.id;

  const eventIndex = todoArray.findIndex(element => {
    return element.id == parentId});

  todoArray[eventIndex].name = newNameVal;
  addToLocalStorage(todoArray);
